...
import pandas as pd
import sys
from phyrvm.config.config import RdRP_DB_PATH, NR_DB_PATH, rRNA_DB_PATH,BBDUK_PATH
from phyrvm.external.fasta_process import Rmdup, Seqkit
from phyrvm.external.reads_tool import Bowtie2, Megahit
from phyrvm.external.blast import Diamond 
from phyrvm.phytree.translate import Translate
from phyrvm.rvm.main_taxon import main_taxon


class Contigs_filter(object):
    ...
    def qc_with_rm_rRNA(self):
        self.logger.info('[contigs_filter] Quality control of sequencing data')
        if len(self.reads) == 2:
        ...
        self.logger.info('[contigs_filter] Remove rRNA')
    ...
    def run(self,options,rm_rRNA_file,filename):
        self.logger.info('[contigs_filter] Use megahit to splice reads into contigs')
        ...
        self.logger.info(f'[contigs_filter] Running diamond blastx to compare {RdRP_DB_PATH}')
        ...
        if os.path.getsize(rdrp_out_fasta) < 1:
            self.logger.warn(f'[contigs_filter] No contigs are filtered out.')
            sys.exit()

        self.logger.info(f'[contigs_filter] Running diamond blastx to compare {NR_DB_PATH}')
        ...
        self.logger.info(f'[contigs_filter] Remove contigs that cannot be translated into longer amino acid contigs.')
        ...
        self.logger.info(f'[contigs_filter] Contigs annotation')
