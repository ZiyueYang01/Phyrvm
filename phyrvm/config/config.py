...
LOG_TASK = 21
phyrvm_path = os.path.abspath(os.path.join(os.path.dirname(__file__),os.path.pardir))


BBDUK_PATH = join(phyrvm_path, 'external','package','bbmap',"bbduk.sh")

RdRP_DB_PATH = join(phyrvm_path, 'data', 'diamond_database', 'RdRP_230330_rmdup')
RdRP_DB_TABLE_PATH = join(phyrvm_path, 'data', 'diamond_database', 'RdRp_expanded_20230330.csv')
Pathogenicity_VIRUS_CSV = join(phyrvm_path, 'data', 'Pathogenicity_VIRUS_CSV.csv')
RdRP_ICTV_NR = join(phyrvm_path, 'data', 'RdRP_ICTV_NR.csv')
ICTV_TAXON_CSV = join(phyrvm_path, 'data', 'ICTV_TAXON.csv')
INTER_RdRP_CLUSTR_PATH = join(phyrvm_path, 'data', 'RdRP_tree_230328')
TAXONKIT_DB = join(phyrvm_path, 'data', 'taxonkit_db')


try:
    # expandvars is required to transform things like $HOME
    PHYRVM_DB_PATH = os.path.expandvars(os.environ['PHYRVM_DB_PATH'])
except KeyError:
    print("The 'PHYRVM_DB_PATH' environment variable is not defined.".center(80) + '\n')
    sys.exit(1)


NR_DB_PATH = PHYRVM_DB_PATH+"/NR/nr"
NT_DB_PATH = PHYRVM_DB_PATH+"/NT/nt"
rRNA_DB_PATH = PHYRVM_DB_PATH+'/rRNA/rRNA_cutout_ref'
PROT_ACC2TAXID = PHYRVM_DB_PATH+'/accession2taxid/prot.accession2taxid'
